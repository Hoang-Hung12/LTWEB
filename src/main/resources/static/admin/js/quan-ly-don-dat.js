document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateStatCards() {
    document.getElementById('cnt-cho').textContent  = allDons.filter(d => d.trangThai === 'Chờ xác nhận thanh toán').length;
  ...
  function buildActions(d) {
    ...
    if (tt === 'Chờ xác nhận thanh toán') {
      html += `<button class="btn btn-info btn-sm me-1" onclick="xacNhanThanhToan('${d.maDon}')" title="Xác nhận thanh toán"><i class="fas fa-money-check-dollar"></i> Xác nhận TT</button>`;
  ...
  async function xacNhanThanhToan(maDon) {
    const ok = await askConfirm(`Xác nhận đã nhận chuyển khoản cho đơn ${maDon}?`);
    if (!ok) return;
    try {
      ...
      updateStatCards();
      applyFilter();
      showToast(`Đã xác nhận thanh toán cho đơn ${maDon}`, 'success');
      if (viewingDon && viewingDon.maDon === maDon) viewDon(maDon);
    } catch (e) {
      showToast('Lỗi xác nhận thanh toán: ' + (e.message || 'Vui lòng thử lại!'), 'danger');
    }
  }
  ...
  function viewDon(maDon) {
    ...
    document.getElementById('d-pttt').textContent    = d.phuongThucThanhToan === 'CHUYEN_KHOAN' ? 'Chuyển khoản' : 'Tiền mặt (đã chuyển cọc)';
    document.getElementById('d-admin-tt').textContent = d.maAdminXacNhanThanhToan || '—';
    document.getElementById('d-time-tt').textContent = d.thoiGianXacNhanThanhToan ? new Date(d.thoiGianXacNhanThanhToan).toLocaleString('vi-VN') : '—';
    ...
    if (tt === 'Chờ xác nhận thanh toán') {
      actDiv.innerHTML = `
        <button class="btn btn-info flex-fill" onclick="xacNhanThanhToan('${d.maDon}');closeDetail()">
          <i class="fas fa-money-check-dollar me-1"></i>Xác nhận thanh toán
        </button>
